...
def top_deliver(df1, top_asc):
    """Esta função tem o objetivo de retornar um dataframe com os top entregadores mais lentos ou mais rápidos
        Input:
            1. df1 - Dataframe: Base de dados que possui as informações necessárias
            2. top_asc: Tipo de ordenação
                True = Top entregadores mais rápidos
                False = Top entregadores mais lentos
        Output:
            Dataframe: retorna um dataframe com o top 10 entregadores mais lentos por cidade
            """
...
with tab1:
    ...
    with st.container():
        ...
        with col2:
            ...
            df_aux = ( df1.loc[:, colunas]
                          ...
                          .agg({'Delivery_person_Ratings' : ['mean', 'std']}) )
            df_aux.columns = ('delivery_svg', 'delivery_std')
            df_aux = df_aux.reset_index()
            st.dataframe(df_aux)
    ...
    with st.container():
        ...
        with col1:
            df3 = top_deliver(df1, True)
            st.markdown('##### Top entregadores mais rápidos')
            st.dataframe(df3)
        
        with col2:
            df3 = top_deliver(df1, False)
