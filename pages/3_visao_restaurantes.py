...
#Funções
#------------------------------------------
#funções de gráficos e dataframes

def avg_std_festival( df1, festival, op):
    """Esta função tem o objetivo de mostrar a média ou o desvio padrão do tempo de entrega nos periodos de festival e não festival
        Input:
            1. df1: Dataframe que contém os dados necessários
            2. festival: Status do festival
                Yes: período de festival
                No: período sem festival
            3. op:
                avg_time: Tempo médio de entrega
                std_time: desvío padrão de entrega
        Output:
            1. df_aux: dataframe que contém as informações necessárias para a exibição da métrica requerida
    """
...
def clean_code(df1):
    """ Esta função tem a resposabilidade de limpar o dataframe
        ...
        2. Tipagem correta dos dados
        3. Remoção de espaços das variáveis de tipo texto
        4. Formatação da coluna de datas
        5. Limpeza da coluna tempo (remoção do texto do conteúdo numérico)
...
#Realziando a limpeza do dataframe e atribuindo ao dataframe df1
df1 = clean_code(df)
...
with tab1:
    ...
    #tmcf        
        with col3:
            df_aux = avg_std_festival(df1, 'Yes', 'avg_time')
            cols = ['Time_taken(min)', 'Festival']
            st.metric('TMCF', df_aux)
    #vmcf
        with col4:
            df_aux = avg_std_festival(df1, 'Yes', 'std_time')
            cols = ['Time_taken(min)', 'Festival']
            st.metric('VMCF', df_aux)
    #tmsf
        with col5:
            df_aux = avg_std_festival(df1, 'No', 'avg_time')
            cols = ['Time_taken(min)', 'Festival']
            st.metric('TMSF', df_aux)
    #vmsf
        with col6:
            df_aux = avg_std_festival(df1, 'No', 'std_time')
            cols = ['Time_taken(min)', 'Festival']
